...
<div class="space-y-6">
    ...
    <div class="w-full">
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in-down">
            <div class="p-6 border-b border-gray-50 flex items-center justify-between bg-siakad-bg/30">
                <h3 class="text-lg font-bold text-siakad-purple"><i class="fas fa-plus-circle mr-2 text-siakad-purple"></i> <span id="form-title">Tambah Sanksi</span></h3>
            </div>
            <div class="p-8">
                <form id="form-sanksi">
                    @csrf
                    <div class="flex flex-col md:flex-row items-end gap-6">
                        <div class="flex-1 w-full">
                            <label for="sanksi" class="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-2 ml-1">Nama Sanksi</label>
                            <div class="relative group">
                                ...
                                <input type="text" name="sanksi" id="sanksi" required 
                                    ...
                                    placeholder="Contoh: Skor 10 Poin">
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-3 w-full md:w-auto">
                            ...
                            <button type="submit" id="btn"
                                class="flex-1 md:flex-none py-3 px-8 bg-siakad-purple hover:bg-indigo-800 text-white font-bold rounded-xl shadow-lg shadow-indigo-100 transition-all transform hover:-translate-y-0.5 active:translate-y-0">
                                <span>Simpan</span>
                            </button>
    ...
    <div class="w-full">
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
            <div class="p-6 border-b border-gray-50 flex items-center justify-between bg-siakad-bg/30">
                <h3 class="text-lg font-bold text-siakad-purple"><i class="fas fa-list mr-2 text-siakad-purple"></i> Daftar Sanksi Pelanggaran</h3>
            </div>
            <div class="p-8">
                <div class="dt-responsive">
                    <table id="order-table" class="w-full text-sm text-left border-collapse">
                        <thead>
                            <tr class="text-xs font-bold text-gray-500 uppercase tracking-widest border-b border-gray-50">
                                <th class="px-4 py-4 w-20">No</th>
                                <th class="px-4 py-4">Sanksi Pelanggaran</th>
...
<div id="confirmModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="$('#confirmModal').addClass('hidden')"></div>
        ...
        <div class="inline-block align-middle bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse space-y-2 sm:space-y-0 sm:space-x-2 sm:space-x-reverse">
                <button type="button" id="ok_button" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2.5 bg-red-600 text-base font-bold text-white hover:bg-red-700 focus:outline-none sm:w-auto sm:text-sm transition-all">
                    Hapus Data
                </button>
                <button type="button" class="w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2.5 bg-white text-base font-bold text-gray-700 hover:bg-gray-50 focus:outline-none sm:w-auto sm:text-sm transition-all" onclick="$('#confirmModal').addClass('hidden')">
                    Batal
                </button>
            </div>
        </div>
...
<style>
    ...
    /* Hide the default bootstrap modal transition if not needed */
    .modal-backdrop {
...
<script>
    $(document).ready(function () {
        var table = $('#order-table').DataTable({
            ...
            // Default order: latest entries first (if No column is used as proxy or rely on server sort)
            order: [], 
            language: {
                search: "_INPUT_",
                searchPlaceholder: "Cari sanksi...",
                ...
                emptyTable: "Belum ada data sanksi tersedia",
                zeroRecords: "Data sanksi tidak ditemukan"
            },
        ...
        $('#form-sanksi').on('submit', function (event) {
            ...
            var url = (action === 'add') ? "{{ route('admin.pelanggaran.sanksi') }}" : "{{ route('admin.pelanggaran.sanksi-update') }}";
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.errors) {
                        ...
                        $('#sanksi').addClass('ring-2 ring-red-500');
                    }
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#sanksi').val(data.sanksi.name).focus();
                    $('#hidden_id').val(data.sanksi.id);
                    $('#action').val('edit');
                    $('#form-title').text('Edit Sanksi');
        ...
        function resetForm() {
            $('#form-sanksi')[0].reset();
            $('#sanksi').removeClass('ring-2 ring-red-500');
            $('#action').val('add');
            $('#hidden_id').val('');
            $('#form-title').text('Tambah Sanksi');
        ...
        $('#ok_button').click(function () {
            $.ajax({
                url: '/admin/pelanggaran/sanksi/hapus/'+del_id,
                beforeSend: function () {
